...
float getSegInteriorAlpha(int texNum, uint seg)
{
  ...
  for (int i = 0; i < 9; ++i) {
    ...
    vec3 texSamplingPos = row * u_texSamplingDirsForSegOutline[0] + col * u_texSamplingDirsForSegOutline[1];
...
void main()
{
  ...
  for (int i = 0; i < N; ++i) {
    ...
    bool imgMask =
      !(any(lessThan(fs_in.v_texCoord[i], MIN_IMAGE_TEXCOORD + u_texSampleSize[i])) ||
