...
float sampleGlyphCoverage(
    sampler2D atlas, int gIdx, int glyphCount,
    vec2 uvCell, vec2 slotSizePx, float sdfPadding, float pixDistScale,
    vec2 cellSizePx)
{
...
vec4 asciiComposite(
    float glyph, vec4 srcPM,
    vec3 fgColor, vec3 bgColor, float bgAlpha, bool useColormap)
{
