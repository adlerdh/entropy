    ...
    glm::vec2 cellSizePx)
{
    ...
    for (int sy = 0; sy < sampleH; ++sy) {
        ...
        for (int sx = 0; sx < sampleW; ++sx) {
            ...
            const float sdfVal = px(r0, c0) * (1.0f - fx) * (1.0f - fy)
                               + px(r0, c1) *          fx  * (1.0f - fy)
                               + px(r1, c0) * (1.0f - fx) *          fy
                               + px(r1, c1) *          fx  *          fy;
...
std::vector<int> buildCoverageRankOrder(const std::vector<float>& coverage)
{
    ...
    std::stable_sort(order.begin(), order.end(),
        [&](int a, int b){ return coverage[static_cast<size_t>(a)] < coverage[static_cast<size_t>(b)]; });
    return order;
}
...
std::vector<int> buildLumLut(const std::vector<float>& coverage)
{
    ...
    for (int bin = 0; bin < 256; ++bin) {
        const int rank = static_cast<int>(std::round(
            static_cast<float>(bin) / 255.0f * static_cast<float>(N - 1)));
...
std::array<float, kSpatialK> computePerRegionMax(const std::vector<GlyphProfile>& profiles) {
    ...
    for (auto& v : mx) v = std::max(v, 1e-6f);
    return mx;
}

void normalizeGlyphProfilesInPlace(std::vector<GlyphProfile>& profiles,
    ...
    uint8_t onedgeValue)
{
    ...
    for (char ch : charset) {
        ...
        uint8_t* bmp = stbtt_GetCodepointBitmap(
            &fontInfo, scale, scale, codepoint, &bmpW, &bmpH, nullptr, nullptr);
        int filled = 0;
        if (bmp) {
        ...
        const float fillFraction = (Gw * Gh > 0)
            ? static_cast<float>(filled) / static_cast<float>(Gw * Gh)
            : 0.0f;
        ...
        uint8_t* sdf = stbtt_GetCodepointSDF(
            ...
            padding, onedgeValue, pixDistScale,
            &sdfW, &sdfH, &xoff, &yoff);
        ...
        if (sdf) {
            ...
            for (int row = 0; row < sdfH; ++row) {
                ...
                for (int col = 0; col < sdfW; ++col) {
                    ...
                    slotPixels[static_cast<size_t>(dstRow * slotW + dstCol)]
                        = sdf[row * sdfW + col];
                }
            }
    ...
    std::stable_sort(glyphs.begin(), glyphs.end(),
        [](const BakedGlyph& a, const BakedGlyph& b) {
