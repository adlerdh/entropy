...
DataManager::DataManager() : m_impl(std::make_unique<Impl>()) {}
...
bool DataManager::unloadLabelTable(const UID& labelsUID)
{
  ...
  if (m_impl->m_labelsRecords.erase(labelsUID) > 0) {
    ...
    for (auto it = std::begin(m_impl->m_parcelUid_to_labelsUid); it != std::end(m_impl->m_parcelUid_to_labelsUid);) {
...
bool DataManager::setSlideOrder(const std::list<UID>& orderedSlideUids)
{
  ...
  if (
    m_impl->m_orderedSlideUids.size() != orderedSlideUids.size() ||
...
std::optional<long> DataManager::orderedImageColorMapIndex(const UID& uid)
{
  ...
  auto it =
    std::find(std::begin(m_impl->m_orderedImageColorMapUids), std::end(m_impl->m_orderedImageColorMapUids), uid);
