...
bool CameraInteractionHandler::doHandleMouseDoubleClickEvent(const QMouseEvent*, const Viewport&, const Camera&)
{
  return false;
}

bool CameraInteractionHandler::doHandleMouseMoveEvent(const QMouseEvent* event, const Viewport& viewport, const Camera&)
{
    ...
    switch (m_mouseMoveMode) {
      ...
      case MouseMoveMode::ZoomAboutPoint: {
        ...
        if (m_zoomSynchronizer) {
          ...
          const glm::vec3 worldCenterPos = camera::world_O_ndc(*camera, glm::vec3{ndcPos, ndcCrosshairsOrigin.z});
...
bool CameraInteractionHandler::doHandleWheelEvent(const QWheelEvent* event, const Viewport& viewport, const Camera&)
{
  if (!m_crosshairsOriginProvider || !m_cameraProvider) {
    return false;
  }
  ...
  switch (m_primaryMode) {
    ...
    case CameraInteractionMode::Zoom: {
      if (camera->isOrthographic()) {
        ...
        const glm::vec4 cameraCrosshairsPos = camera->camera_O_world() * glm::vec4{m_crosshairsOriginProvider(), 1.0f};
...
bool CameraInteractionHandler::doHandlePanGesture(const QPanGesture*, const Viewport&, const Camera&)
{
  bool handled = false;
  return handled;
}
  ...
  const Camera&)
{
  ...
  const glm::vec2 ndcPos =
    (camera->isOrthographic()) ? camera::ndc2d_O_mouse(viewport, centerPoint) : glm::vec2{0.0f, 0.0f};
...
bool CameraInteractionHandler::doHandleSwipeGesture(const QSwipeGesture* gesture, const Viewport&, const Camera&)
{
  bool handled = false;
