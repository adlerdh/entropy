...
void createDistanceMaps(const Image& image, const uuids::uuid& imageUid, float downsamplingFactor, AppData& data)
{
  ...
  // Create ITK images with float components from which distance maps and noise estimates are
  // computed
  using TItkImageComp = float;
  ...
  for (uint32_t comp = 0; comp < image.header().numComponentsPerPixel(); ++comp) {
    ...
    spdlog::debug(
      "Computing Euclidean distance map for image {} using thresholds {} and {}",
      imageUid,
      threshLow,
      threshHigh);
    ...
    spdlog::debug(
      "Created distance map ({}x{}x{} voxels) to foreground region [{}, {}] of component {} "
      "of image {}",
      distMapSize.x,
      distMapSize.y,
      distMapSize.z,
