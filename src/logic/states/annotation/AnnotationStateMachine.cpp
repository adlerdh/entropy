...
std::optional<uuid> ASM::ms_hoveredViewUid = std::nullopt;
std::optional<uuid> ASM::ms_selectedViewUid = std::nullopt;
std::optional<uuid> ASM::ms_growingAnnotUid = std::nullopt;
...
bool AnnotationStateMachine::createNewGrowingPolygon(const ViewHit& hit)
{
  ...
  std::optional<uuid> annotUid;

  try {
    const std::string name =
      std::string("Annotation ") + std::to_string(ms_appData->annotationsForImage(*activeImageUid).size());

    const glm::vec4 color{activeImage->settings().borderColor(), 1.0f};

    annotUid = ms_appData->addAnnotation(*activeImageUid, Annotation(name, color, subjectPlaneEquation));
...
bool AnnotationStateMachine::addVertexToGrowingPolygon(const ViewHit& hit)
{
  ...
  if (growingAnnot->numBoundaries() > 0) {
    ...
    const bool hasMoreThanTwoVertices = (growingAnnot->getBoundaryVertices(OUTER_BOUNDARY).size() >= 2);
    ...
    for (const auto& vertex : hitVertices) {
      ...
      if (
        *ms_growingAnnotUid == vertex.first && FIRST_VERTEX_INDEX == vertex.second && !growingAnnot->isClosed() &&
  ...
  const auto projectedPoint = growingAnnot->addSubjectPointToBoundary(OUTER_BOUNDARY, subjectPlanePoint);
...
void AnnotationStateMachine::completeGrowingPoylgon(bool closePolgon)
{
  ...
  if (closePolgon) {
    ...
    const bool hasThreeOrMoreVertices = (growingAnnot->getBoundaryVertices(OUTER_BOUNDARY).size() >= 3);
...
void AnnotationStateMachine::insertVertex()
{
  ...
  if (N - 1 == *ms_selectedVertex) {
    ...
    if (1 == N) {
      ...
      // so insert a new vertex after the last vertex that continues the line from the previous
      // vertex.
      const size_t prevVertexIndex = *ms_selectedVertex - 1;

      const std::optional<glm::vec2> prevVertex = annot->polygon().getBoundaryVertex(OUTER_BOUNDARY, prevVertexIndex);
      ...
      const std::optional<glm::vec2> firstVetex = annot->polygon().getBoundaryVertex(OUTER_BOUNDARY, firstVertexIndex);
    ...
    const std::optional<glm::vec2> nextVertex = annot->polygon().getBoundaryVertex(OUTER_BOUNDARY, nextVertexIndex);
...
void AnnotationStateMachine::moveSelectedVertex(const ViewHit& prevHit, const ViewHit& currHit)
{
  ...
  if (glm::all(
        glm::epsilonEqual(prevHit.worldPos_offsetApplied, currHit.worldPos_offsetApplied, glm::epsilon<float>())))
  {
    return;
  }
...
void AnnotationStateMachine::pasteAnnotation() const
{
  ...
  {
    spdlog::warn(
      "The normal vector of the view plane and the normal vector of the "
...
void AnnotationStateMachine::flipSelectedAnnotation(const FlipDirection& flipDirection)
{
  ...
  auto transformFromAnnotPlaneToViewClip =
    ...
    const glm::vec4 clipPoint = helper::clip_T_world(selectedView->camera()) *
                                activeImage->transformations().worldDef_T_subject() * glm::vec4{subjectPoint, 1.0f};
  ...
  for (const glm::vec2& annotVertex : vertices) {
    ...
    const glm::vec4 subjectVertexFlipped = activeImage->transformations().subject_T_worldDef() *
                                           helper::world_T_clip(selectedView->camera()) * clipVertexFlipped;

    // Project flipped vertex back into annotation plane:
    const glm::vec2 annotPlaneVertexFlipped =
      annot->projectSubjectPointToAnnotationPlane(glm::vec3{subjectVertexFlipped / subjectVertexFlipped.w});
...
std::vector<std::pair<uuid, size_t> > AnnotationStateMachine::findHitVertices(const ViewHit& hit)
{
  // Distance threshold for hitting a vertex (in pixels)
  static constexpr float sk_distThresh_inPixels = 6.0f;

  static const std::vector<std::pair<uuid, size_t> > sk_empty;
  ...
  const glm::vec2 mmPerPixel =
    helper::worldPixelSize(ms_appData->windowData().viewport(), hit.view->camera(), hit.view->viewClip_T_windowClip());
...
std::vector<uuid> AnnotationStateMachine::findHitPolygon(const ViewHit& hit)
{
  static const std::vector<uuid> sk_empty;
    ...
    if (vertexIndex) {
      ...
      if (annot && annot->numBoundaries() > 0) {
        if (annot->getBoundaryVertices(OUTER_BOUNDARY).size() > *vertexIndex) {
          ...
          spdlog::warn("Cannot select invalid vertex at index {} for annotation {}", *vertexIndex, annotUid);
        }
      }
    }
    else {
...
FSM_INITIAL_STATE(state::annot::AnnotationStateMachine, state::annot::AnnotationOffState)
