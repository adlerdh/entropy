...
enum ImGuiFileBrowserFlags_
{
  ...
  ImGuiFileBrowserFlags_MultipleSelection = 1 << 7, // allow user to select multiple files. this will
                                                    // hide ImGuiFileBrowserFlags_EnterNewFilename
...
inline void ImGui::FileBrowser::Display()
{
  ...
  if (SmallButton("*")) {
    ...
    for (auto& name : selectedFilenames_) {
      auto it = std::find_if(fileRecords_.begin(), fileRecords_.end(), [&](const FileRecord& record) {
...
inline bool ImGui::FileBrowser::IsExtensionMatched(const fs::path& _extension) const
{
#ifdef _WIN32
