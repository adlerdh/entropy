...
public:
  virtualGizmoBaseClass()
    : pos(0)
    , delta(0)
    , tbControlButton(evLeftButton)
    , tbRotationButton(evRightButton)
  ...
  {
    if (
      (button == tbControlButton) && pressed &&
      (tbControlModifiers ? tbControlModifiers & mod : tbControlModifiers == mod))
    {
      tbActive = true;
      activateMouse(x, y);
    }
    ...
    if ((button == tbRotationButton) && pressed) {
      if (xRotationModifier & mod) {
        tbActive = true;
        rotationVector = tVec3(T(1), T(0), T(0));
        activateMouse(x, y);
      }
      else if (yRotationModifier & mod) {
        tbActive = true;
        rotationVector = tVec3(T(0), T(1), T(0));
        activateMouse(x, y);
      }
      else if (zRotationModifier & mod) {
        tbActive = true;
        rotationVector = tVec3(T(0), T(0), T(1));
        activateMouse(x, y);
      }
    }
    else if ((button == tbRotationButton) && !pressed) {
      deactivateMouse();
      rotationVector = tVec3(T(1));
      tbActive = false;
    }
  }
  ...
  void updateGizmo()
  {
    ...
    T AdotB = dot(a, b);
    T angle = acos(
      AdotB > T(1) ? T(1) : (AdotB < -T(1) ? -T(1) : AdotB)); // clamp need!!! corss float is approximate to FLT_EPSILON
  ...
  //  Apply rotation
  //////////////////////////////////////////////////////////////////
  inline void applyRotation(tMat4& m)
  {
    m = m * mat4_cast(qtV);
  }
  ...
  ////////////////////////////////////////////////////////////////////////////
  void setIdleRotSpeed(T f)
  {
    qIdleSpeedRatio = f;
  }
  T getIdleRotSpeed()
  {
    return qIdleSpeedRatio;
  }
...
protected:
  ...
  // UI commands that this virtualGizmo responds to (defaults to left mouse button with no modifier
  // key)
...
public:
  //////////////////////////////////////////////////////////////////
  virtualGizmo3DClass()
    : dollyControlButton(evRightButton)
    , panControlButton(evMiddleButton)
    , dollyControlModifiers(evNoModifier)
    , panControlModifiers(evNoModifier)
    , dollyActive(false)
    , panActive(false)
  {
  }
  ...
  {
    mouse(button, mod, pressed, T(x), T(y));
  }
  void mouse(vgButtons button, vgModifiers mod, bool pressed, T x, T y)
  {
    VGIZMO_BASE_CLASS::mouse(button, mod, pressed, x, y);
    if (
      button == dollyControlButton && pressed &&
      (dollyControlModifiers ? dollyControlModifiers & mod : dollyControlModifiers == mod))
    {
      dollyActive = true;
    ...
    if (
      button == panControlButton && pressed &&
      (panControlModifiers ? panControlModifiers & mod : panControlModifiers == mod))
    {
      panActive = true;
      this->activateMouse(x, y);
    }
  ...
  {
    return panControlButton;
  }
  int getPanControlModifier()
  {
    return panControlModifiers;
  }

  // Sensitivity for Wheel movements -> Normalized: less < 1 < more
  //////////////////////////////////////////////////////////////////
  void setNormalizedWheelScale(T scale)
  {
    wheelScale = scale * constWheelScale;
  }
  void setWheelScale(T scale)
  {
    wheelScale = scale;
  }
  T getNormalizedWheelScale()
  {
    return wheelScale / constWheelScale;
  }
  T getWheelScale()
  {
    return wheelScale;
  ...
  //////////////////////////////////////////////////////////////////
  void setNormalizedDollyScale(T scale)
  {
    dollyScale = scale * constDollyScale;
  }
  void setDollyScale(T scale)
  {
    dollyScale = scale;
  }
  T getNormalizedDollyScale()
  {
    return dollyScale / constDollyScale;
  }
  T getDollyScale()
  {
    return dollyScale;
  ...
  //////////////////////////////////////////////////////////////////
  void setNormalizedPanScale(T scale)
  {
    panScale = scale * constPanScale;
  }
  void setPanScale(T scale)
  {
    panScale = scale;
  }
  T getNormalizedPanScale()
  {
    return panScale / constPanScale;
  }
  T getPanScale()
  {
    return panScale;
  ...
  //////////////////////////////////////////////////////////////////
  void setNormalizedDistScale(T scale)
  {
    distScale = scale * constDistScale;
  }
  void setDistScale(T scale)
  {
    distScale = scale;
  }
  T getNormalizedDistScale()
  {
    return distScale / constDistScale;
  }
  T getDistScale()
  {
    return distScale;
  ...
  {
    return tVec3(pan.x, pan.y, dolly.z);
  }
