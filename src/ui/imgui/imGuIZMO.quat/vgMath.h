...
public:
  ...
  Mat3(T v0x, T v0y, T v0z, T v1x, T v1y, T v1z, T v2x, T v2y, T v2z)
    : v{VEC3_T(v0x, v0y, v0z), VEC3_T(v1x, v1y, v1z), VEC3_T(v2x, v2y, v2z)}
  {
  }
  ...
  {
    return {v[0] + m.v[0], v[1] + m.v[1], v[2] + m.v[2]};
  }
  Mat3 operator-(const Mat3& m) const
  {
    return {v[0] - m.v[0], v[1] - m.v[1], v[2] - m.v[2]};
...
#undef M
  ...
  {
    return {m00 * v.x + m10 * v.y + m20 * v.z, m01 * v.x + m11 * v.y + m21 * v.z, m02 * v.x + m12 * v.y + m22 * v.z};
  }
  explicit operator const T*() const
  {
    return &m00;
...
public:
  ...
  Mat4(T v0x, T v0y, T v0z, T v0w, T v1x, T v1y, T v1z, T v1w, T v2x, T v2y, T v2z, T v2w, T v3x, T v3y, T v3z, T v3w)
    : v{VEC4_T(v0x, v0y, v0z, v0w), VEC4_T(v1x, v1y, v1z, v1w), VEC4_T(v2x, v2y, v2z, v2w), VEC4_T(v3x, v3y, v3z, v3w)}
  {
  }
...
#define M(X, Y) (m##X * m.m##Y)
  ...
  {
    return {
      M(00, 00) + M(10, 01) + M(20, 02) + M(30, 03),
...
TEMPLATE_TYPENAME_T inline MAT4_T mat4_cast(QUAT_T const& q)
{
  return {mat3_cast(q)};
}
...
inline uint32_t floatBitsToUint(float const v)
{
  return *((uint32_t*)(&v));
}
// dot
//////////////////////////
TEMPLATE_TYPENAME_T inline T dot(const VEC2_T& v0, const VEC2_T& v1)
{
  return v0.x * v1.x + v0.y * v1.y;
}
TEMPLATE_TYPENAME_T inline T dot(const VEC3_T& v0, const VEC3_T& v1)
{
  return v0.x * v1.x + v0.y * v1.y + v0.z * v1.z;
}
TEMPLATE_TYPENAME_T inline T dot(const VEC4_T& v0, const VEC4_T& v1)
{
  return v0.x * v1.x + v0.y * v1.y + v0.z * v1.z + v0.w * v1.w;
}
TEMPLATE_TYPENAME_T inline T dot(const QUAT_T& q0, const QUAT_T& q1)
{
  return q0.x * q1.x + q0.y * q1.y + q0.z * q1.z + q0.w * q1.w;
}
// cross
//////////////////////////
TEMPLATE_TYPENAME_T inline T cross(const VEC2_T& u, const VEC2_T& v)
{
  return u.x * v.y - v.x * u.y;
}
TEMPLATE_TYPENAME_T inline VEC3_T cross(const VEC3_T& u, const VEC3_T& v)
{
  return {u.y * v.z - u.z * v.y, u.z * v.x - u.x * v.z, u.x * v.y - u.y * v.x};
}
// length
//////////////////////////
TEMPLATE_TYPENAME_T inline T length(const VEC2_T& v)
{
  return sqrt(dot(v, v));
}
TEMPLATE_TYPENAME_T inline T length(const VEC3_T& v)
{
  return sqrt(dot(v, v));
}
TEMPLATE_TYPENAME_T inline T length(const VEC4_T& v)
{
  return sqrt(dot(v, v));
}
TEMPLATE_TYPENAME_T inline T length(const QUAT_T& q)
{
  return sqrt(dot(q, q));
}
// distance
//////////////////////////
TEMPLATE_TYPENAME_T inline T distance(const VEC2_T& v0, const VEC2_T& v1)
{
  return length(v1 - v0);
}
TEMPLATE_TYPENAME_T inline T distance(const VEC3_T& v0, const VEC3_T& v1)
{
  return length(v1 - v0);
}
TEMPLATE_TYPENAME_T inline T distance(const VEC4_T& v0, const VEC4_T& v1)
{
  return length(v1 - v0);
}
// abs
//////////////////////////
TEMPLATE_TYPENAME_T inline T tAbs(T x)
{
  return x >= T(0) ? x : -x;
}
TEMPLATE_TYPENAME_T inline VEC2_T abs(const VEC2_T& v)
{
  return {tAbs(v.x), tAbs(v.y)};
}
TEMPLATE_TYPENAME_T inline VEC3_T abs(const VEC3_T& v)
{
  return {tAbs(v.x), tAbs(v.y), tAbs(v.z)};
}
TEMPLATE_TYPENAME_T inline VEC4_T abs(const VEC4_T& v)
{
  return {tAbs(v.x), tAbs(v.y), tAbs(v.z), tAbs(v.w)};
}
TEMPLATE_TYPENAME_T inline QUAT_T abs(const QUAT_T& q)
{
  return {tAbs(q.w), tAbs(q.x), tAbs(q.y), tAbs(q.z)};
}
// sign
//////////////////////////
TEMPLATE_TYPENAME_T inline T sign(const T v)
{
  return v > T(0) ? T(1) : (v < T(0) ? T(-1) : T(0));
}
// normalize
//////////////////////////
TEMPLATE_TYPENAME_T inline VEC2_T normalize(const VEC2_T& v)
{
  return v / length(v);
}
TEMPLATE_TYPENAME_T inline VEC3_T normalize(const VEC3_T& v)
{
  return v / length(v);
}
TEMPLATE_TYPENAME_T inline VEC4_T normalize(const VEC4_T& v)
{
  return v / length(v);
}
TEMPLATE_TYPENAME_T inline QUAT_T normalize(const QUAT_T& q)
{
  return q / length(q);
}
// mix
//////////////////////////
TEMPLATE_TYPENAME_T inline T mix(const T x, const T y, const T a)
{
  return x + (y - x) * a;
}
TEMPLATE_TYPENAME_T inline VEC2_T mix(const VEC2_T& x, const VEC2_T& y, const T a)
{
  return x + (y - x) * a;
}
TEMPLATE_TYPENAME_T inline VEC3_T mix(const VEC3_T& x, const VEC3_T& y, const T a)
{
  return x + (y - x) * a;
}
TEMPLATE_TYPENAME_T inline VEC4_T mix(const VEC4_T& x, const VEC4_T& y, const T a)
{
  return x + (y - x) * a;
}
// pow
//////////////////////////
TEMPLATE_TYPENAME_T inline VEC2_T pow(const VEC2_T& b, const VEC2_T& e)
{
  return {::pow(b.x, e.x), ::pow(b.y, e.y)};
}
TEMPLATE_TYPENAME_T inline VEC3_T pow(const VEC3_T& b, const VEC3_T& e)
{
  return {::pow(b.x, e.x), ::pow(b.y, e.y), ::pow(b.z, e.z)};
}
TEMPLATE_TYPENAME_T inline VEC4_T pow(const VEC4_T& b, const VEC4_T& e)
{
  return {::pow(b.x, e.x), ::pow(b.y, e.y), ::pow(b.z, e.z), ::pow(b.w, e.w)};
}
// value_ptr
//////////////////////////
TEMPLATE_TYPENAME_T inline T* value_ptr(const VEC2_T& v)
{
  return const_cast<T*>(&v.x);
}
TEMPLATE_TYPENAME_T inline T* value_ptr(const VEC3_T& v)
{
  return const_cast<T*>(&v.x);
}
TEMPLATE_TYPENAME_T inline T* value_ptr(const VEC4_T& v)
{
  return const_cast<T*>(&v.x);
}
TEMPLATE_TYPENAME_T inline T* value_ptr(const QUAT_T& q)
{
  return const_cast<T*>(&q.x);
}
TEMPLATE_TYPENAME_T inline T* value_ptr(const MAT3_T& m)
{
  return const_cast<T*>(&m.m00);
}
TEMPLATE_TYPENAME_T inline T* value_ptr(const MAT4_T& m)
{
  return const_cast<T*>(&m.m00);
}
// transpose
...
TEMPLATE_TYPENAME_T inline QUAT_T inverse(QUAT_T const& q)
{
  return QUAT_T(q.w, -q.x, -q.y, -q.z) / dot(q, q);
}
TEMPLATE_TYPENAME_T inline MAT3_T inverse(MAT3_T const& m)
{
  T invDet =
    T(1) / (m.m00 * (M(11, 22) - M(21, 12)) - m.m10 * (M(01, 22) - M(21, 02)) + m.m20 * (M(01, 12) - M(11, 02)));
  return MAT3_T(
           (M(11, 22) - M(21, 12)),
           -(M(01, 22) - M(21, 02)),
           (M(01, 12) - M(11, 02)),
...
{
  const T c0 = M(22, 33) - M(32, 23);
  VEC4_T f0(c0, c0, M(12, 33) - M(32, 13), M(12, 23) - M(22, 13));
  const T c1 = M(21, 33) - M(31, 23);
  VEC4_T f1(c1, c1, M(11, 33) - M(31, 13), M(11, 23) - M(21, 13));
  const T c2 = M(21, 32) - M(31, 22);
  VEC4_T f2(c2, c2, M(11, 32) - M(31, 12), M(11, 22) - M(21, 12));
  const T c3 = M(20, 33) - M(30, 23);
  VEC4_T f3(c3, c3, M(10, 33) - M(30, 13), M(10, 23) - M(20, 13));
  const T c4 = M(20, 32) - M(30, 22);
  VEC4_T f4(c4, c4, M(10, 32) - M(30, 12), M(10, 22) - M(20, 12));
  const T c5 = M(20, 31) - M(30, 21);
  VEC4_T f5(c5, c5, M(10, 31) - M(30, 11), M(10, 21) - M(20, 11));
#undef M
...
// external operators
//////////////////////////
TEMPLATE_TYPENAME_T inline VEC2_T operator*(const T s, const VEC2_T& v)
{
  return v * s;
}
TEMPLATE_TYPENAME_T inline VEC3_T operator*(const T s, const VEC3_T& v)
{
  return v * s;
}
TEMPLATE_TYPENAME_T inline VEC4_T operator*(const T s, const VEC4_T& v)
{
  return v * s;
}
TEMPLATE_TYPENAME_T inline QUAT_T operator*(const T s, const QUAT_T& q)
{
  return q * s;
}
...
TEMPLATE_TYPENAME_T inline VEC3_T operator*(const VEC3_T& v, const QUAT_T& q)
{
  return inverse(q) * v;
}
// translate / scale
...
// quat angle/axis
//////////////////////////
TEMPLATE_TYPENAME_T inline QUAT_T angleAxis(T const& a, VEC3_T const& v)
{
  return QUAT_T(cos(a * T(0.5)), v * sin(a * T(0.5)));
}
TEMPLATE_TYPENAME_T inline T angle(QUAT_T const& q)
{
  return acos(q.w) * T(2);
}
TEMPLATE_TYPENAME_T inline VEC3_T axis(QUAT_T const& q)
{
  const T t1 = T(1) - q.w * q.w;
  if (t1 <= T(0)) return VEC3_T(0, 0, 1);
  const T t2 = T(1) / sqrt(t1);
  return VEC3_T(q.x * t2, q.y * t2, q.z * t2);
}
// trigonometric
//////////////////////////
TEMPLATE_TYPENAME_T inline T radians(T d)
{
  return d * T(0.0174532925199432957692369076849);
}
TEMPLATE_TYPENAME_T inline T degrees(T r)
{
  return r * T(57.295779513082320876798154814105);
}
TEMPLATE_TYPENAME_T inline T pi()
{
  return T(3.1415926535897932384626433832795029);
}
TEMPLATE_TYPENAME_T inline T one_over_pi()
{
...
#else
  VEC3_T k = normalize(tgt - pov), i = normalize(cross(k, up)), j = cross(i, k);
  k = -k;
#endif
  return {
    i.x,
...
// perspectiveFov
//////////////////////////
TEMPLATE_TYPENAME_T inline MAT4_T perspectiveFov(T fovy, T w, T h, T n, T f)
{
  return perspective(fovy, w / h, n, f);
}
// frustrum
//////////////////////////
