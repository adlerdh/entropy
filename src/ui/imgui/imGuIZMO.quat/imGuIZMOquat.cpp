...
inline vec3& adjustPlane(vec3& coord)
{
  ...
  coord.x = (coord.x) * .5f + .5f +
            (coord.x > 0 ? -imguiGizmo::planeThickness : imguiGizmo::planeThickness) * imguiGizmo::solidResizeFactor;
...
{
  return (
    (axis == imguiGizmo::axisIsY) ? vec3(-v.y, v.x, v.z) :    // rotation Z 90'
...
bool imguiGizmo::drawFunc(const char* label, float size)
{
  ...
  const float arrowStartingPoint =
    (axesOriginType & imguiGizmo::sphereAtOrigin)
      ? sphereRadius * solidResizeFactor
      : ((axesOriginType & imguiGizmo::cubeAtOrigin) ? cubeSize * solidResizeFactor : cylRadius * .5f);
  // if modeDual... leave space for draw light arrow
  vec3 resizeAxes(
    ((drawMode & modeDual) && (axesResizeFactor.x > .75f)) ? vec3(.75f, axesResizeFactor.y, axesResizeFactor.z)
                                                           : axesResizeFactor);
  ...
  if (io.KeyCtrl) {
    vgMods |= vg::evControlModifier;
    vgModsActive = true;
  }
  if (io.KeyAlt) {
    vgMods |= vg::evAltModifier;
    vgModsActive = true;
  }
  if (io.KeyShift) {
    vgMods |= vg::evShiftModifier;
    vgModsActive = true;
  }
  if (io.KeySuper) {
    vgMods |= vg::evSuperModifier;
    vgModsActive = true;
  }
  ...
  auto getTrackball = [&](quat& q) {
    ...
    if (drawMode & modePanDolly || std::abs(io.MouseWheel) > 1e-6f) {
      ...
      if ((!track.isDollyActive() && !track.isPanActive() && (std::abs(io.MouseWheel) < 1e-6f)))
        q = track.getRotation();
      else
        posPanDolly = track.getPosition();
    }
    else {
  ...
  if (ImGui::IsItemActive()) {
    ...
    // if((drawMode&modeDual) && ImGui::IsMouseDragging(2)) { getTrackball(qtV); getTrackball(qtV2);
    // } // middle if dual mode... move together
    ...
    if (highlighted && (drawMode & modeDual) && ImGui::IsMouseDragging(1))
      getTrackball(qtV2);
    else if (highlighted && (drawMode & modeDual) && ImGui::IsMouseDragging(2)) {
      getTrackball(qtV);
      getTrackball(qtV2);
    }
#ifndef IMGUIZMO_USE_ONLY_ROT
    else if (highlighted && (std::abs(io.MouseWheel) > 1e-6f))
      getTrackball(qtV);
#endif
  ...
  ImVec2 uv[4];
  ImU32 col[4]; // buffers to storetransformed vtx & col for PrimVtx & PrimQuadUV
  ...
  auto normalizeToControlSize = [&](float x, float y) {
    return controlPos + ImVec2(x, -y) * halfSquareSize +
           ImVec2(halfSquareSize, halfSquareSize); // drawing from 0,0 .. no borders
  };

  auto returnSizeFromRatio = [&](float ratio) {
    return squareSize * ratio;
  };

  //////////////////////////////////////////////////////////////////
  auto addTriangle = [&]() { // test cull dir
    if (cross(vec2(uv[1].x - uv[0].x, uv[1].y - uv[0].y), vec2(uv[2].x - uv[0].x, uv[2].y - uv[0].y)) > 0) {
      uv[1] = uv[2] = uv[0];
    }

    for (int i = 0; i < 3; i++)
      draw_list->PrimVtx(uv[i], wpUV, col[i]);
  };

  //////////////////////////////////////////////////////////////////
  auto addQuad = [&](ImU32 colLight) { // test cull dir
    if (cross(vec2(uv[1].x - uv[0].x, uv[1].y - uv[0].y), vec2(uv[3].x - uv[0].x, uv[3].y - uv[0].y)) > 0) {
      uv[3] = uv[1] = uv[2] = uv[0];
    }
  ...
  auto drawSphere = [&]() {
    ...
    for (auto itVtx = sphereVtx.begin(); itVtx != sphereVtx.end();) {
      for (int h = 0; h < 3; h++, itTess++) {
        ...
        // col[h] = colorLightedY(sphereCol[i++], (-sizeSphereRadius.5f + (coord.z*coord.z) /
        // (sizeSphereRadius*sizeSphereRadius)), coord.z);
      }
      addTriangle();
    }
  };
  ...
  auto drawAxes = [&](int side) {
    for (int n = 0; n < 4; n++) { // Arrow: 2 Cone -> (Surface + cap) + 2 Cyl -> (Surface + cap)
      for (int arrowAxis = 0; arrowAxis < 3; arrowAxis++) { // draw 3 axes
        vec3 arrowCoord(0.0f, 0.0f, 0.0f);
        arrowCoord[arrowAxis] = 1.0f; // rotate on 3 axis (arrow -> X, Y, Z ) in base to current arrowAxis
        ...
        for (auto itVtx = ptrVtx->begin(), itNorm = (arrowNorm + i)->begin(); itVtx != ptrVtx->end();) { // for all Vtx
          ...
          for (int h = 0; h < 3; h++) {
            ...
            if ((skipCone && coord.x <= 0) || (!showFullAxes && (coord.x < arrowStartingPoint)))
            ...
            col[h] = addLightEffect(
              vec4(float(arrowAxis == axisIsX), float(arrowAxis == axisIsY), float(arrowAxis == axisIsZ), 1.0),
              norm.z,
              coord.z);
          }
          addTriangle();
  ...
  auto drawComponent = [&](const int idx, const quat& q, ptrFunc func) {
    ...
    for (auto itVtx = ptrVtx->begin(), itNorm = (arrowNorm + idx)->begin(); itVtx != ptrVtx->end();) {
      ...
      for (int h = 0; h < 3; h++) {
        ...
        col[h] = addLightEffect(
          vec4(directionColor.x, directionColor.y, directionColor.z, 1.0f),
          norm.z,
          coord.z > 0 ? coord.z : coord.z * .5f);
      }
      addTriangle();
    }
  };
  ...
  auto dirArrow = [&](const quat& q, int mode) {
    ...
    if (arrowCoord.z <= 0) {
      for (int i = 0; i < 4; i++)
        drawComponent(i, q, func);
      if (mode & modeDirPlane) drawPlane();
    }
    else {
      if (mode & modeDirPlane) drawPlane();
      for (int i = 3; i >= 0; i--)
  ...
  auto spotArrow = [&](const quat& q, const float arrowCoordZ) {
    if (arrowCoordZ > 0) {
      drawComponent(CONE_SURF, q, adjustSpotCone);
      drawComponent(CONE_CAP, q, adjustSpotCone);
      drawComponent(CYL_SURF, q, adjustSpotCyl);
      drawComponent(CYL_CAP, q, adjustSpotCyl);
    }
    else {
      drawComponent(CYL_CAP, q, adjustSpotCyl);
      drawComponent(CYL_SURF, q, adjustSpotCyl);
  ...
  auto draw3DSystem = [&]() {
    drawAxes(backSide);
    if (axesOriginType & sphereAtOrigin)
      drawSphere();
    else if (axesOriginType & cubeAtOrigin)
      drawCube();
    drawAxes(frontSide);
  };
...
#define CENTER_HELPER_Y -.85f
  ...
  auto drawRotationHelper = [&]() {
    ...
    const ImU32 color = (vgMods & vg::evShiftModifier)     ? 0xff0000ff
                        : (vgMods & vg::evControlModifier) ? 0xff00ff00
                                                           : 0xffff0000;
  ...
  auto drawPanHelper = [&]() {
    ...
    draw_list->AddTriangleFilled(
      ImVec2(center.x, center.y + lenLine + halfLen),
    ...
    draw_list->AddTriangleFilled(
      ...
      color);
    draw_list->AddTriangleFilled(
      ImVec2(center.x + lenLine + halfLen, center.y),
  ...
  auto drawDollyHelper = [&]() {
    ...
    draw_list->AddTriangleFilled(
      ...
      color);
    draw_list->AddTriangleFilled(
      ImVec2(center.x, center.y - lenLine),
...
#define T(t) sphereTess.push_back(t)
  ...
  for (int i = 1; i < parallels - 1; i++, angle += incAngle) {
    // int div =8;
    z0 = z1;
    z1 = cosf(angle) * radius;
    r0 = r1;
    r1 = sinf(angle) * radius;
...
void imguiGizmo::buildCone(const float x0, const float x1, const float radius, const int slices)
{
  ...
  arrowVtx[CONE_CAP].clear();
  arrowNorm[CONE_CAP].clear();
  arrowVtx[CONE_SURF].clear();
  arrowNorm[CONE_SURF].clear();
...
void imguiGizmo::buildCylinder(const float x0, const float x1, const float radius, const int slices)
{
  ...
  arrowVtx[CYL_CAP].clear();
  arrowNorm[CYL_CAP].clear();
  arrowVtx[CYL_SURF].clear();
  arrowNorm[CYL_SURF].clear();
