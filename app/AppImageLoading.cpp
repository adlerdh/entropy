...
void EntropyApp::continueRasterImageHeaderPreflight()
{
  ...
  if (m_pendingRasterImageHeaderContext == RasterImageHeaderContext::Project && m_pendingRasterProject) {
    ...
    allImages.reserve(project_image_sequence::size(*m_pendingRasterProject));
    ...
    while (m_pendingRasterImageIndex < project_image_sequence::size(*m_pendingRasterProject)) {
      serialize::Image* image = project_image_sequence::at(*m_pendingRasterProject, m_pendingRasterImageIndex);
      if (!image) {
  ...
  bool applyToAll)
{
  ...
  if (m_pendingRasterImageHeaderContext == RasterImageHeaderContext::Project && m_pendingRasterProject) {
    ...
    if (applyToAll) {
      ...
      m_pendingRasterImageIndex = project_image_sequence::size(*m_pendingRasterProject);
    }
    else if (serialize::Image* image = project_image_sequence::at(*m_pendingRasterProject, m_pendingRasterImageIndex)) {
      applyToSerializedImage(*image);
      m_pendingRasterImageIndex++;
    }
    continueRasterImageHeaderPreflight();
...
bool EntropyApp::setReferenceImage(const uuids::uuid& imageUid)
{
  ...
  m_callbackHandler.recenterViews(m_data.settings().recenteringMode(), true, true, true, true, true);
  GlfwWrapper::postEmptyEvent();
...
bool EntropyApp::removeImage(const uuids::uuid& imageUid)
{
  ...
  rendering::RenderResources& renderResources = m_data.renderResources();
  renderResources.removeImage(imageUid);
  m_data.renderDerivedData().removeImage(imageUid);

  for (const auto& segUid : segUids) {
    if (!m_data.seg(segUid)) {
      renderResources.removeSegmentation(segUid);
    }
  }

  for (const auto& defUid : defUids) {
    if (!m_data.def(defUid)) {
      renderResources.removeImage(defUid);
      m_data.renderDerivedData().removeImage(defUid);
    }
  }
