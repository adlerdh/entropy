...
void main()
{
  ...
  vec3 centerNormal = safeNormalize(centerEncodedNormal.xyz * 2.0 - 1.0, vec3(0.0, 0.0, 1.0));
  ...
  for (int y = -2; y <= 2; ++y) {
    for (int x = -2; x <= 2; ++x) {
      ...
      vec3 normal = safeNormalize(encodedNormal.xyz * 2.0 - 1.0, centerNormal);
  ...
  float powered = pow(clamp(filtered, 0.0, 1.0), max(u_power, 1.0e-3));
  outOcclusion = 1.0 - clamp((1.0 - powered) * max(u_contrast, 0.0), 0.0, 1.0);
}
