 ...
 *
 * The GPU uses limited-precision sampler weight / coordinate math in the fixed-function texture
 * unit. This is not limited precision of the stored texel values themselves.
 *
 *
 * For hardware trilinear filtering, the GPU does:
 * 1. Convert normalized texture coordinates to texel space (multiply by size, apply wrapping/clamp,
 * subtract 0.5).
 ...
 * (or equivalently for the lerp weights). A common precision is 8 fractional bits (described as
 * "24.8" or "8-bit sub-texel precision"), meaning there are only 256 distinct weight values between
 * adjacent texels along an axis. That produces stair step artifacts: the weights only change in
 * 1/256 increments, so the output changes in visible quantized steps when the texture values are
 * smooth/high precision.
 *
