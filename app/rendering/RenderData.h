...
struct RenderData
{
  ...
  Quad m_quad; //!< Shared full-viewport/image-plane quad

  Circle m_circle; //!< Shared circle mesh used by overlay rendering
  ...
  std::unordered_map<uuids::uuid, GLTexture> m_colormapTextures; //!< Uploaded color-map textures keyed by color-map UID
  ...
  std::unordered_map<uuids::uuid, ImageUniforms> m_uniforms; //!< Cached image uniforms keyed by image UID
  ...
  float m_intensityProjectionSlabThickness; //!< Intensity projection slab thickness in millimeters
  ...
  float m_xrayIntensityWindow; //!< Window width used for contrast adjustment in X-ray intensity projection mode

  float m_xrayIntensityLevel; //!< Window level used for contrast adjustment in X-ray intensity projection mode

  float m_xrayEnergyKeV; //!< Monoenergetic photon energy, in keV, used by X-ray intensity projection mode

  /// Linear attenuation coefficient of water, in cm^-1, for the current X-ray photon energy.
  float m_waterMassAttenCoeff;

  float m_airMassAttenCoeff; //!< Linear attenuation coefficient of air, in cm^-1, for the current X-ray photon energy
  ...
  bool m_showCrosshairs; //!< Whether crosshairs are rendered in 2D views
  ...
  glm::vec4 m_anatomicalLabelColor; //!< Anatomical label text color as non-premultiplied RGBA

  bool m_showAnatomicalLabels; //!< Whether anatomical direction labels are rendered in 2D views
  ...
  float m_anatomicalLabelScale = 1.0f; //!< Scale multiplier for anatomical direction label text

  /// Anatomical label vocabulary used for direction labels.
  AnatomicalLabelType m_anatomicalLabelType = AnatomicalLabelType::Human;

  bool m_showScaleBars; //!< Whether scale bars are rendered in 2D views
  ...
  glm::vec4 m_scaleBarColor; //!< Scale bar color as non-premultiplied RGBA

  ScaleBarPosition m_scaleBarPosition; //!< Scale bar placement within each view

  ScaleBarOrientation m_scaleBarOrientation; //!< Scale bar orientation

  ScaleBarTicks m_scaleBarTicks; //!< Scale bar tick style
  ...
  float m_scaleBarMarginPx; //!< Scale bar margin from the selected view edge in screen pixels

  bool m_showLightboxOffsetLabels; //!< Whether lightbox offset labels are rendered

  glm::vec4 m_lightboxOffsetLabelColor; //!< Lightbox offset label color as non-premultiplied RGBA

  bool m_renderFrontFaces; //!< Whether front-facing image box faces are considered during 3D raycasting

  bool m_renderBackFaces; //!< Whether back-facing image box faces are considered during 3D raycasting
  ...
  float m_adaptiveRaycastTargetFrameRate; //!< Target frame rate for adaptive raycast sampling

  float m_adaptiveRaycastEffectiveSamplingFactor; //!< Effective raycast sampling factor chosen by adaptive sampling
  ...
  bool m_reverseThreeDRotateAboutEye; //!< Whether 3D rotate-about-eye drag direction is reversed

  glm::vec4 m_threeDCameraFrustumColor; //!< 3D camera frustum overlay color as non-premultiplied RGBA
  ...
  SegMaskingForRaycasting m_segMasking; //!< Current segmentation masking behavior for raycasting

  /// Global segmentation outline style.
  SegmentationOutlineStyle m_segOutlineStyle = SegmentationOutlineStyle::Disabled;

  float m_segInteriorOpacity = 0.2f; //!< Opacity of segmentation interiors when outline rendering is enabled

  float m_segInterpCutoff = 0.5f; //!< Threshold used when rendering linearly interpolated segmentation samples

  /// Shared color-map and masking parameters used by comparison and local metric render modes.
  struct MetricParams
  {
    size_t m_colorMapIndex = 0; //!< Index of the color map used to render metric values
    ...
    bool m_invertCmap = false; //!< Whether the metric color map is inverted

    bool m_cmapContinuous = true; //!< Whether metric rendering samples the color map continuously
    ...
    bool m_doMasking = false; //!< Whether metric values should only be computed inside the current mask

    bool m_volumetric = false; //!< Reserved volumetric metric flag; currently not implemented
  };

  MetricParams m_squaredDifferenceParams; //!< Display parameters for squared-difference comparison mode

  MetricParams m_localNccParams; //!< Display parameters for local normalized cross-correlation mode

  MetricParams m_localLinearResidualParams; //!< Display parameters for local linear residual mode

  MetricParams m_jointHistogramParams; //!< Display parameters for joint histogram mode
  ...
  bool m_overlayMagentaCyan; //!< Whether overlay comparison uses cyan/magenta/white instead of red/green/yellow

  glm::ivec2 m_quadrants; //!< Quadrant split direction flags used by quadrant comparison mode

  bool m_useSquare; //!< Whether difference mode uses squared difference instead of absolute difference

  bool m_asciiEnabled = false; //!< Whether ASCII rendering is enabled for image views

  /// ASCII cell size in screen pixels.
  glm::vec2 m_asciiCellSizePx{8.f, 16.f};

  int m_asciiCharsetIndex = 0; //!< Index of the active ASCII character set
  ...
  float m_asciiBgAlpha = 1.f; //!< ASCII background opacity

  bool m_asciiUseColormap = false; //!< Whether ASCII foreground color is sampled from the image color map
  ...
  float m_flashlightRadius; //!< Flashlight radius in normalized view coordinates
  ...
  bool m_manualFramerateLimiter = false; //!< Whether the manual frame-rate limiter is enabled
  ...
  struct AnnotationParams
  {
    ...
    bool hidePolygonVertices = false; //!< Whether polygon vertices are hidden for vector annotations
  };
  ...
  LandmarkParams m_globalLandmarkParams; //!< Global landmark rendering parameters

  AnnotationParams m_globalAnnotationParams; //!< Global annotation rendering parameters

  SliceIntersectionParams m_globalSliceIntersectionParams; //!< Global slice intersection overlay parameters
