 ...
 * @brief
 */
class TexturedMesh final : public DrawableBase, public ITexturable3d, public ObjectCounter<TexturedMesh>
{
public:
  TexturedMesh(
    std::string name,
...
Draw the first object (the one that should appear behind the other object) by depth testing but not
depth writing Draw the second object by depth testing and depth writing. This won't cause z-fighting
since we didn't write any depth in step 1. Draw the first object by only writing to the depth buffer
and not to the color buffer. This makes sure the depth buffer is up to date for any pixels that are
