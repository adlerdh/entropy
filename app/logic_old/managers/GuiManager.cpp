  ...
  GetterType<IDrawable*> overlayProvider)
{
  ...
  auto renderer =
    std::make_unique<DepthPeelRenderer>(name.str(), shaderActivator, uniformsProvider, rootProvider, overlayProvider);
...
{
  if (
    !m_viewUidAndTypeProvider || !m_viewWidgetSetter || !m_interactionPackProvider || !m_rootDrawableProvider ||
...
void GuiManager::setParcellationSelectionsPublisher(gui::ParcellationSelection_msgFromUi_PublisherType publisher)
{
  if (m_refImageEditorDock) {
...
void GuiManager::setParcellationSelectionsResponder(gui::ParcellationSelections_msgToUi_ResponderType responder)
{
  if (m_refImageEditorDock) {
...
void GuiManager::sendParcellationPropertiesPartialToUi(const gui::ParcellationPropertiesPartial_msgToUi& msg)
{
  if (m_refImageEditorDock) {
...
void GuiManager::sendParcellationPropertiesCompleteToUi(const gui::ParcellationPropertiesComplete_msgToUi& msg)
{
  if (m_refImageEditorDock) {
...
void GuiManager::setParcellationLabelsPartialPublisher(gui::ParcellationLabelsPartial_msgFromUi_PublisherType publisher)
{
  if (m_refImageEditorDock) {
...
void GuiManager::setParcellationLabelsCompleteResponder(gui::ParcellationLabelsComplete_msgToUi_ResponderType responder)
{
  if (m_refImageEditorDock) {
...
void GuiManager::createViewWidgets()
{
  ...
  if (
    !m_viewUidAndTypeProvider || !m_interactionPackProvider || !m_rootDrawableProvider || !m_overlayDrawableProvider ||
    !m_viewWidgetSetter)
  {
    return;
  }
  ...
  for (const auto& viewUidAndType : m_viewUidAndTypeProvider()) {
    ...
    auto renderer = createDdpRenderer(viewUid, m_shaderActivator, m_uniformsProvider, rootProvider, overlayProvider);
    ...
    auto viewWidget =
      new gui::ViewWidget(viewUid, glWidget, m_viewScrollBarValuesBroadcaster, m_viewSliceSliderValueBroadcaster);
