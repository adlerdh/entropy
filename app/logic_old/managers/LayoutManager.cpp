...
LayoutManager::LayoutManager(const std::string& layoutConfig) : m_impl(std::make_unique<Impl>(layoutConfig)) {}
...
LayoutManager::Impl::Impl(const std::string& layoutConfig)
  : m_parsedLayouts(), m_orderedLayoutUids(), m_layoutData(), m_viewTypes(), m_viewSplitters()
{
  try {
