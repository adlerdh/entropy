...
{
  if (
    !m_actionManager || !m_assemblyManager || !m_connectionManager || !m_dataManager || !m_guiManager ||
...
void AppController::initialize()
{
  ...
  for (const UID& layoutUid : m_layoutManager->getOrderedLayoutUids()) {
    ...
    m_guiManager->insertViewLayoutTab(tabIndex++, layoutData.m_containerWidget, layoutData.m_displayName);
  }
}
...
void AppController::createUiConnections()
{
  ...
  m_guiManager->setParcellationSelectionsPublisher(
    std::bind(&ParcellationDataUiMapper::setParcellationSelections_fromUi, m_parcelDataUiMapper.get(), _1));
  m_guiManager->setParcellationPropertiesPartialPublisher(
    std::bind(&ParcellationDataUiMapper::setParcellationPropertiesPartial_fromUi, m_parcelDataUiMapper.get(), _1));
  m_guiManager->setParcellationLabelsPartialPublisher(
    std::bind(&ParcellationDataUiMapper::setParcellationLabelsPartial_fromUi, m_parcelDataUiMapper.get(), _1));
  ...
  m_guiManager->setSlideCommonPropertiesPartialPublisher(
    std::bind(&SlideStackDataUiMapper::setSlideCommonPropertiesPartial_fromUi, m_slideStackDataUiMapper.get(), _1));
  m_guiManager->setSlideHeaderPartialPublisher(
    std::bind(&SlideStackDataUiMapper::setSlideHeaderPartial_fromUi, m_slideStackDataUiMapper.get(), _1));
  m_guiManager->setSlideViewDataPartialPublisher(
    std::bind(&SlideStackDataUiMapper::setSlideViewDataPartial_fromUi, m_slideStackDataUiMapper.get(), _1));
  m_guiManager->setSlideTxDataPartialPublisher(
    std::bind(&SlideStackDataUiMapper::setSlideTxDataPartial_fromUi, m_slideStackDataUiMapper.get(), _1));
  ...
  m_guiManager->setParcellationPropertiesCompleteResponder(
    std::bind(&ParcellationDataUiMapper::getParcellationPropertiesComplete_msgToUi, m_parcelDataUiMapper.get(), _1));
  m_guiManager->setParcellationLabelsCompleteResponder(
    std::bind(&ParcellationDataUiMapper::getParcellationLabelsComplete_msgToUi, m_parcelDataUiMapper.get(), _1));
  ...
  m_guiManager->setSlideCommonPropertiesCompleteResponder(
    std::bind(&SlideStackDataUiMapper::getSlideCommonPropertiesComplete_msgToUi, m_slideStackDataUiMapper.get()));
  m_guiManager->setSlideHeaderCompleteResponder(
    std::bind(&SlideStackDataUiMapper::getSlideHeaderComplete_msgToUi, m_slideStackDataUiMapper.get(), _1));
  m_guiManager->setSlideViewDataCompleteResponder(
    std::bind(&SlideStackDataUiMapper::getSlideViewDataComplete_msgToUi, m_slideStackDataUiMapper.get(), _1));
  m_guiManager->setSlideTxDataCompleteResponder(
    std::bind(&SlideStackDataUiMapper::getSlideTxDataComplete_msgToUi, m_slideStackDataUiMapper.get(), _1));
}
...
void AppController::loadProject(serialize::HZeeProject project)
{
  ...
  for (const auto& slide : project.m_slides) {
    if (
      const auto slideUid =
        m_actionManager->loadSlide(slide.m_fileName, slide.m_slideStack_T_slide.autoTranslateToTopOfStack()))
    {
...
void AppController::loadBuiltInImageColorMaps(const std::vector<std::string>& colorMapFileNames)
{
  ...
  if (m_globalContext && m_globalContext->makeCurrent(&m_surface)) {
    ...
    // which is not provided as a CSV file in the res directory.
    data::loadDefaultGreyscaleColorMap(*m_dataManager);

    // Next, load all color maps from the res directory
...
void AppController::testCreateRefImageLandmark()
{
  ...
  auto testLandmarkGroupRecord1 =
    std::make_shared<LandmarkGroupRecord>(std::move(cpuRecord1), std::make_unique<EmptyGpuRecord>());
  ...
  auto testLandmarkGroupRecord2 =
    std::make_shared<LandmarkGroupRecord>(std::move(cpuRecord2), std::make_unique<EmptyGpuRecord>());

  if (const auto activeImageUid = m_dataManager->activeImageUid()) {
    const auto group1Uid = m_dataManager->insertRefImageLandmarkGroupRecord(*activeImageUid, testLandmarkGroupRecord1);

    const auto group2Uid = m_dataManager->insertRefImageLandmarkGroupRecord(*activeImageUid, testLandmarkGroupRecord2);
...
void AppController::testCreateSlideLandmark()
{
  ...
  if (auto activeSlideUid = m_dataManager->activeSlideUid()) {
    ...
    const auto groupUid = m_dataManager->insertSlideLandmarkGroupRecord(*activeSlideUid, testPointLandmarkRecord);
...
void AppController::testCreateSlideAnnotation()
{
  ...
  auto annot1Record = std::make_shared<SlideAnnotationRecord>(std::move(annot1CpuRecord), std::move(annot1GpuRecord));

  auto annot2Record = std::make_shared<SlideAnnotationRecord>(std::move(annot2CpuRecord), std::move(annot2GpuRecord));

  auto annot3Record = std::make_shared<SlideAnnotationRecord>(std::move(annot3CpuRecord), std::move(annot3GpuRecord));
