...
{
public:
  MeshInfo(MeshSource meshSource, MeshPrimitiveType primitiveType, boost::variant<double, uint32_t> scalarValue);
...
private:
  ...
  {
    uint32_t operator()(double) const
    {
      return 0;
    }
    uint32_t operator()(uint32_t s) const
    {
      return s;
    }
  };
