...
void Camera::setProjection(std::unique_ptr<Projection> projectionArg)
{
  if (projectionArg) {
    m_projection = std::move(projectionArg);
  }
}
...
void Camera::set_camera_T_anatomy(glm::mat4 camera_T_anatomyArg)
{
  ...
  if (
    glm::epsilonNotEqual(camera_T_anatomyArg[0][3], 0.0f, EPS) ||
    glm::epsilonNotEqual(camera_T_anatomyArg[1][3], 0.0f, EPS) ||
    glm::epsilonNotEqual(camera_T_anatomyArg[2][3], 0.0f, EPS) ||
    glm::epsilonNotEqual(camera_T_anatomyArg[3][3], 1.0f, EPS))
  {
    spdlog::debug("Cannot set camera_T_anatomy to {} because it is not affine", glm::to_string(camera_T_anatomyArg));
    return;
  }

  m_camera_T_anatomy = camera_T_anatomyArg;
}
...
void Camera::set_start_T_world(glm::mat4 start_T_worldArg)
{
  m_start_T_world = start_T_worldArg;
}
