#include "logic/old/slideio/SlideHelper.h"
...
std::optional<AABB<float> > slideStackAABBoxInWorld(
  weak_record_range_t<SlideCpuRecord> slideRecordRange,
  const glm::mat4& world_O_slideStack)
{
  ...
  for (const auto& weak_record : slideRecordRange) {
    ...
    const std::array<glm::vec3, 8> stackCornersOfSlide = slideCornersInStack(*record);
...
float slideStackHeight(weak_record_range_t<SlideCpuRecord> slideRecords)
{
  ...
  for (const auto& weak_record : slideRecords) {
    ...
    const auto corners = slideio::slideCornersInStack(*record);
...
float slideStackPositiveExtent(weak_record_range_t<SlideCpuRecord> slideRecords)
{
  float maxZ = 0.0f;
