...
{
  auto camera = std::make_unique<Camera>(std::move(cameraProjection), cameraStartFrameProvider);
...
const std::unordered_map<gui::ViewType, CrosshairsType> InteractionManager::smk_viewTypeToDefaultCrosshairsTypeMap{
...
const std::unordered_map<gui::ViewType, CameraType> InteractionManager::smk_viewTypeToDefaultCameraTypeMap = {
...
const std::unordered_map<CameraType, camera::ProjectionType> InteractionManager::smk_cameraTypeToProjectionTypeMap = {
...
CameraStartFrameType InteractionManager::getDefaultCameraStartFrameType(const CameraType& cameraType)
{
  return smk_cameraTypeToDefaultStartFrameTypeMap.at(cameraType);
}

LinkedFrameType InteractionManager::getDefaultLinkedStartFrameType(const CameraType& cameraType)
{
...
void InteractionManager::addInteractionPackForView(const UID& viewUid, const gui::ViewType& viewType)
{
  ...
  auto startFrameProvider = [viewType, this]() {
    return computeStartFrame(viewType);
  };
  ...
  for (auto& pack : m_interactionPacks) {
    if (pack.second) {
      if (auto camera = pack.second->getCamera()) {
        ...
        // This is the default near distance, but it is updated every time the camera moves
...
CoordinateFrame InteractionManager::computeStartFrame(const gui::ViewType& viewType) const
{
  ...
  CoordinateFrame anatomicalFrame(sk_origin, getDefaultAnatomicalRotation(getDefaultCameraStartFrameType(cameraType)));
  ...
  switch (linkedFrameType) {
    case LinkedFrameType::SlideStack: {
      if (m_slideStackFrameProvider) {
        ...
        //            baseFrame.setFrameToWorldRotation(
        //            m_slideStackFrameProvider().world_O_frame_rotation() );
      }
      break;
    }
    case LinkedFrameType::Crosshairs:
...
void InteractionManager::setActiveSlideViewDirection(const InteractionManager::ActiveSlideViewDirection& dir)
{
  switch (dir) {
    case ActiveSlideViewDirection::TopToBottomSlide: {
      m_viewTypeToCameraTypeMap[gui::ViewType::Stack_ActiveSlide] = CameraType::SlideActive_TopToBottomSlide;
      break;
    }
    case ActiveSlideViewDirection::BottomToTopSlide: {
      m_viewTypeToCameraTypeMap[gui::ViewType::Stack_ActiveSlide] = CameraType::SlideActive_BottomToTopSlide;
      break;
    }
  }
}
