...
user_preferences::RenderPreferences makeNonDefaultRenderPreferences()
{
  ...
  preferences.imagePlaneAmbient = 0.42f;
  preferences.imagePlaneDiffuse = 0.83f;
  preferences.imagePlaneSpecular = 0.51f;
  preferences.imagePlaneShininess = 48.0f;
  ...
  const user_preferences::RenderPreferences& expected)
{
  ...
  CHECK(actual.imagePlaneAmbient == Catch::Approx(expected.imagePlaneAmbient));
  CHECK(actual.imagePlaneDiffuse == Catch::Approx(expected.imagePlaneDiffuse));
  CHECK(actual.imagePlaneSpecular == Catch::Approx(expected.imagePlaneSpecular));
  CHECK(actual.imagePlaneShininess == Catch::Approx(expected.imagePlaneShininess));
...
void resetProjectOwnedSettings(AppSettings& settings, user_preferences::RenderPreferences& renderPreferences)
{
  ...
  renderPreferences.imagePlaneAmbient = defaults.imagePlaneAmbient;
  renderPreferences.imagePlaneDiffuse = defaults.imagePlaneDiffuse;
  renderPreferences.imagePlaneSpecular = defaults.imagePlaneSpecular;
  renderPreferences.imagePlaneShininess = defaults.imagePlaneShininess;
