...
TEST_CASE("default image filtering keeps all images when render-all is enabled", "[windowing][image_selection]")
{
  const std::list imageUids{uuidFromIndex(1), uuidFromIndex(2), uuidFromIndex(3)};

  const auto filtered = windowing::filteredDefaultRenderedImages(imageUids, true, {1});

  CHECK(asVector(filtered) == asVector(imageUids));
}

TEST_CASE("default image filtering keeps preferred zero-based image positions", "[windowing][image_selection]")
{
  const std::list imageUids{uuidFromIndex(1), uuidFromIndex(2), uuidFromIndex(3), uuidFromIndex(4)};

  const auto filtered = windowing::filteredDefaultRenderedImages(imageUids, false, {0, 2});

  CHECK(asVector(filtered) == std::vector{uuidFromIndex(1), uuidFromIndex(3)});
}

TEST_CASE("selected images reorder to match current image order and drop missing UIDs", "[windowing][image_selection]")
{
  ...
  const auto reordered = windowing::reorderSelectedImages(selected, ordered);

  CHECK(asVector(reordered) == std::vector{uuidFromIndex(2), uuidFromIndex(1)});
}

TEST_CASE("selected image reordering can limit metric selections", "[windowing][image_selection]")
{
  const std::list selected{uuidFromIndex(1), uuidFromIndex(2), uuidFromIndex(3)};
  const std::vector ordered{uuidFromIndex(3), uuidFromIndex(2), uuidFromIndex(1)};

  const auto reordered = windowing::reorderSelectedImages(selected, ordered, 2);
