...
createDistanceMapImage(const Image& image, uint32_t component, float downsamplingFactor)
{
  ...
  if (!distMapItkImage) {
    spdlog::warn("Unable to create distance map for component {}", component);
    return std::nullopt;
  }

  const std::string displayName =
    std::string("Dist map for comp ") + std::to_string(component) + " of '" + image.settings().displayName() + "'";
...
std::vector<DistanceMapImageResult> createDistanceMapImages(const Image& image, float downsamplingFactor)
{
  ...
  for (uint32_t component = 0; component < image.header().numComponentsPerPixel(); ++component) {
    if (auto result = createDistanceMapImage(image, component, downsamplingFactor)) {
