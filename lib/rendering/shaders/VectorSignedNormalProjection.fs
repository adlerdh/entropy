...
uniform float u_projectionScale;           // native-unit scale for mapping projection to [-1, 1]
uniform vec3 u_viewNormal_subject;         // current view normal in subject/LPS coordinates
...
void main()
{
  ...
  float signedProjection = dot(vectorValue, normalizeOr(u_viewNormal_subject, vec3(0.0, 0.0, 1.0)));
  float normalizedProjection = clamp(signedProjection / max(u_projectionScale, 1.0e-6), -1.0, 1.0);

  vec3 negativeColor = vec3(0.10, 0.35, 1.00);
  vec3 zeroColor = vec3(0.92, 0.92, 0.92);
  vec3 positiveColor = vec3(1.00, 0.16, 0.10);
  vec3 color = normalizedProjection < 0.0 ? mix(zeroColor, negativeColor, -normalizedProjection)
                                          : mix(zeroColor, positiveColor, normalizedProjection);

  float alpha = u_imgOpacity * mix(0.35, 1.0, abs(normalizedProjection));
