...
vec3 cameraPosition(vec2 uv, float depth)
{
  ...
  return camera.xyz / (abs(camera.w) > 1.0e-12 ? camera.w : 1.0);
}
...
void main()
{
  ...
  vec3 normal = safeNormalize(u_camera_T_worldNormal * (encodedNormal.xyz * 2.0 - 1.0), vec3(0.0, 0.0, 1.0));
  float rotation = pixelRotation(pixelCoord);
  vec3 helperAxis = abs(normal.z) < 0.999 ? vec3(0.0, 0.0, 1.0) : vec3(0.0, 1.0, 0.0);
  vec3 tangent = safeNormalize(cross(helperAxis, normal), vec3(1.0, 0.0, 0.0));
  ...
  for (int i = 0; i < kMaxSampleCount; ++i) {
    ...
    vec3 sampleNdc = sampleClip.xyz / max(sampleClip.w, 1.0e-12);
    ...
    float radiusMm = max(u_radiusMm, 1.0e-3);
    float rangeWeight = 1.0 - smoothstep(radiusMm * 0.5, radiusMm, separation);
    float bias = max(0.002 * radiusMm, 0.001);
  ...
  outOcclusion = clamp(1.0 - max(u_strength, 0.0) * normalized, 0.0, 1.0);
}
