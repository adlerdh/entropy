...
TEST_CASE("settings pages stay in the intended navigation order", "[ui][settings]")
{
  ...
  CHECK(pages[1].page == GuiData::SettingsTab::Rendering);
  CHECK(pages[1].label == std::string("3D Rendering"));
  CHECK(pages[2].page == GuiData::SettingsTab::Interface);
  ...
  CHECK(settings::settingsPageLabel(GuiData::SettingsTab::Rendering) == std::string("3D Rendering"));
